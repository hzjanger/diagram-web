...
option = {
    ...
    tooltip: {
        // trigger:'axis',
        backgroundColor: 'rgba(90,216,255,0.7)',

        formatter: function () {
            var colorNormal = '#35ED8C'
            var colorFault  = '#FE4D53'
             var colorContent1  = colorNormal
             var colorContent2  = colorFault
             var colorContent3  = colorNormal
             var colorContent4  = colorFault
            var html = `
  <div style="color: #fff;opacity: 1;padding: 15px;line-height: 35px;">
   <div style="font-weight: 600;font-size: 17px;">HIS系统</div>
   <div style="display: flex;text-align:center;">
     <div style="width: 50px;">健康度</div>
     <div style="margin:0 10px; color: ${colorContent1};">96%</div>
     <div style="width: 50px;">繁忙度</div>
     <div style="margin:0 10px; color:${colorContent2}">96%</div>
   </div>
   <div style="display: flex;text-align:center;">
     <div style="width: 50px;">可用度</div>
     <div style="margin:0 10px; color: ${colorContent3};">96%</div>
     <div style="width: 50px;">故障</div>
     <div style="margin:0 10px; color: ${colorContent4};">96%</div>
   </div>
  </div>
...
function doing() {
    ...
    var x = option.graphic[0].elements[0].rotation;
    if (Math.abs(x) > rotationTotal) {
        option.graphic[0].elements[0].rotation = 0;
    } else {
        option.graphic[0].elements[0].rotation = x - rotationTotal / 120.0;
    }
    //option.series[1].startAngle = option.series[1].startAngle - 1;
    myChart.setOption(option);

    //console.log(option.series[0].data[faultByHourIndex].emphasis)
    //console.log(option.series[0].data[faultByHourIndex].itemStyle)
    //动态显示tootip
    setTimeout(function () {
        myChart.dispatchAction({
            type: 'showTip', // 根据 tooltip 的配置项显示提示框。
            seriesIndex: 0,
            dataIndex: indexArr[faultByHourIndex],
            position: function (e1, e2, e3, e4) {
                //   console.log(e1)
                //   console.log(e2)
                //   console.log(e3)
                //  console.log(e4)
        ...
        if (faultByHourIndex >= indexArr.length) {
            faultByHourIndex = 0;
        }
    }, 10);
}

setInterval(function () { doing();}, 100);
